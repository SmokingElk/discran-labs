    ...
    for (size_t i = 0; i < m; i++) {
        ...
        while (remainder > 0) {
            ...
            if (it == NULL) {
                ...
                if (lastNode != NULL) {
                    lastNode->suffixLink = active.node;
                }
                lastNode = active.node;
            } else {
                ...
                Pos splitEnd = it->start + active.length - 1;
            ...
            if (active.node == tree->root && active.length > 0) {
                active.length--;
                active.edge = pos - remainder + 1;
            } else if (active.node != tree->root) {
