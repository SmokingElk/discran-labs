    ...
    for (size_t i = 0; i < m; i++) {
        ...
        while (remainder > 0) {
            ...
            if (nextNode == NULL) {
                ...
                S3 newLeaf = createS3Node(text + i, (Pos)(&tree->end), leavesCount++);
            ...
            if (active.node != tree->root) {
                // переходим по суффиксной ссылке, если она есть
                // если нет - переходим в корень
    ...
    if (root->leafNum != NOT_LEAF) {
        if (*matchCount >= *matchCap) {
            *matchCap += EXTEND_SIZE;
            matches = (uint32_t*)realloc(matches, *matchCap);
        }
