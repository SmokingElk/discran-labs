...
void findMatches(SuffixTree tree, Pos p, size_t m);
...
void findMatches(SuffixTree tree, Pos p, size_t m) {
    ...
    RingBuffer text = createRingBuffer(m);
    size_t textPos = 0;
    ...
    while (1) {
        ...
        if (readNext) {
            ...
            setRingBuffer(text, textPos++, textLetter);
            textPos+;
        ...
        if (currentNode == tree->root || letter == currentLength) {
            ...
            size_t pos = textPos - (currentNode->end - p) - 1;
            if (endNode == nodeBefore) pos++;

            while (bethaLength > endLength) {
                bethaLength -= endLength;
                pos += endLength;
                endNode = getMap(&endNode->children, getRingBuffer(text, pos));
