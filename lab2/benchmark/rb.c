...
// Вычисляет 32-битный хеш FNV-1a для строки
uint32_t fnv1a_hash(const char *key) {
    uint32_t hash = FNV_OFFSET_BASIS;
    while (*key) {
        hash ^= (uint32_t)(*key++);  // XOR с текущим символом
        hash *= FNV_PRIME;           // Умножение на простое число
    }
    return hash;
}
...
RBTree _searchRBTree(RBTree tree, Key key) {
    while (tree != _NIL) {
        ...
        if (hash != tree->keyHash) {
            tree = (hash < tree->keyHash) ? tree->left : tree->right;
        } else {
            if (strcmp(key, tree->key) == 0) break;
            tree = tree->right; 
        }
    }
...
void UI(RBTree *treeP) {
    ...
    while (fgets(command, MAX_INPUT_LENGTH, stdin)) {
        ...
        case '-':
            ...
            processKey(command + 2, keyLength);

            if (removeRBTree(treeP, command + 2)) {
                printf("%s\n", RESULT_SUCCESS);
            } else {
                printf("%s\n", RESULT_NOT_FOUND);
            }
        ...
        default:
            ...
            processKey(command, keyLength);

            Value foundedValue;

            if (getRBTree(*treeP, command, &foundedValue)) {
